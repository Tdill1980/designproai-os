...
test("Call 8 proof carries deterministic dimensions, five-inch bleed and square feet", () => {
  ...
  assert.ok(built.request.tiles.every((tile) => tile.masterPath.startsWith(`designpro/${tenantKey}/${runId}/proof-masters/`)));
  assert.ok(built.request.tiles.every((tile) => tile.rawFlatPath.startsWith(`designpro/${tenantKey}/${runId}/proof-masters/raw/`)));
