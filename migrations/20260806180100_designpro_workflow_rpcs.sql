...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' AND auth.uid() IS DISTINCT FROM p_owner_id THEN RAISE EXCEPTION 'owner_identity_required'; END IF;
...
DECLARE v_id uuid; v_token uuid := extensions.gen_random_uuid();
BEGIN
  ...
  SELECT s.id INTO v_id FROM public.designpro_workflow_stages s
  ...
  UPDATE public.designpro_workflow_stages SET status='running',attempt=attempt+1,lease_owner=p_worker,lease_token=v_token,
    lease_expires_at=clock_timestamp()+make_interval(secs=>p_lease_seconds),started_at=COALESCE(started_at,clock_timestamp()),updated_at=clock_timestamp()
  WHERE id=v_id;
...
BEGIN
    ...
    IF (p_receipt->>'call')::int<>10 OR p_receipt->>'inventoryContract'<>'designpro.expected-logo-inventory.v1' OR COALESCE((p_receipt->>'exactSetVerified')::boolean,false) IS DISTINCT FROM true OR lower(p_receipt->>'inventoryHash')!~'^[0-9a-f]{64}$'
      ...
      OR EXISTS(SELECT 1 FROM jsonb_array_elements(v_snapshot->'expectedLogoInventory') expected WHERE
        NOT EXISTS(SELECT 1 FROM jsonb_array_elements(p_receipt->'inventory') actual
          WHERE actual->>'identityKey'=expected->>'identityKey' AND lower(actual->>'contentHash')=lower(expected->>'contentHash'))
        OR NOT EXISTS(SELECT 1 FROM jsonb_array_elements(COALESCE(p_artifacts,'[]')) artifact
          WHERE artifact->>'kind'='logo' AND artifact->>'surfaceKey'=expected->>'identityKey'
  ...
  ELSIF v_stage.stage_key='output.verify' THEN v_kind:='output.verified';
  ...
  IF v_stage.stage_key='output.verify' AND NOT EXISTS(
    SELECT 1 FROM jsonb_array_elements(COALESCE(p_artifacts,'[]')) a WHERE a->>'kind'='output' AND lower(a->>'contentHash')~'^[0-9a-f]{64}$'
  ) THEN RAISE EXCEPTION 'verified_output_artifact_required'; END IF;
  ...
  IF v_stage.stage_key='stamp.build' THEN
    IF NOT EXISTS(SELECT 1 FROM public.designpro_stage_receipts q WHERE q.run_id=v_run.id AND q.receipt_kind='final.human-qc' AND q.receipt->>'verifiedBy'=p_receipt->>'verifiedBy')
      OR NOT EXISTS(SELECT 1 FROM jsonb_array_elements(COALESCE(p_artifacts,'[]')) a WHERE a->>'kind'='stamp' AND lower(a->>'contentHash')=lower(p_receipt_hash))
    THEN RAISE EXCEPTION 'final_qc_and_stamp_artifact_required'; END IF;
  END IF;
  ...
  END IF;
  PERFORM public.designpro_sync_run_status(v_run.id); RETURN true;
END $fn$;
...
BEGIN
  ...
  IF auth.role() IS DISTINCT FROM 'service_role' AND (auth.uid() IS NULL OR auth.uid() IS DISTINCT FROM p_actor OR v_run.owner_id IS DISTINCT FROM p_actor) THEN RAISE EXCEPTION 'workflow_owner_required'; END IF;
...
GRANT EXECUTE ON FUNCTION public.claim_designpro_stage(text,integer),public.heartbeat_designpro_stage(uuid,uuid,integer),public.complete_designpro_stage(uuid,uuid,jsonb,jsonb,text,jsonb),public.fail_designpro_stage(uuid,uuid,text,text,boolean),public.designpro_sync_run_status(uuid) TO service_role;
