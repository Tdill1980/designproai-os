...
BEGIN
  ...
  IF auth.role() IS DISTINCT FROM 'service_role' AND auth.uid() IS DISTINCT FROM v_entice.owner_id THEN RAISE EXCEPTION 'workflow_owner_required'; END IF;
...
BEGIN
  ...
  IF auth.role() IS DISTINCT FROM 'service_role' AND auth.uid() IS DISTINCT FROM v_source.owner_id THEN RAISE EXCEPTION 'workflow_owner_required'; END IF;
