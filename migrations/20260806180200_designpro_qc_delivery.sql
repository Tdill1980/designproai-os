...
DECLARE v_stage public.designpro_workflow_stages%ROWTYPE; v_run public.designpro_workflow_runs%ROWTYPE; v_kind text; v_hash text; v_now timestamptz:=clock_timestamp(); v_verified_by text;
BEGIN
  ...
  IF auth.role() IS DISTINCT FROM 'service_role' AND (auth.uid() IS NULL OR auth.uid() IS DISTINCT FROM p_actor) THEN RAISE EXCEPTION 'qc_actor_identity_required'; END IF;
  ...
  ) INTO v_verified_by FROM auth.users WHERE id=p_actor;
  ...
  IF p_stage_key='await_panelpro_preflight_qc' THEN
    ...
    IF NOT p_qc @> '{"dimensionsVerified":true,"sourceRegionsVerified":true,"fiveInchBleed":true,"panelHashesVerified":true,"logoInventoryVerified":true}'::jsonb THEN RAISE EXCEPTION 'panelpro_preflight_evidence_incomplete'; END IF;
    IF NOT EXISTS(
  ...
  ELSE
    ...
    IF NOT p_qc @> '{"outputHashesVerified":true,"printDimensionsVerified":true,"colorModeVerified":true}'::jsonb THEN RAISE EXCEPTION 'final_qc_evidence_incomplete'; END IF;
...
BEGIN
  ...
  IF auth.role() IS DISTINCT FROM 'service_role' AND v_run.owner_id IS DISTINCT FROM auth.uid() THEN RAISE EXCEPTION 'workflow_owner_required'; END IF;
