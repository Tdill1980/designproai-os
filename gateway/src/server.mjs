  ...
  return createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && url.pathname === "/api/layerize/run") {
        if (!cfg.internalRuntimeUrl || cfg.workerSecret.length < 32) {
          return json(res, 503, { error: "layerize_service_unavailable" });
        }
        const body = await readBody(req);
        const outputMode = String(body?.outputMode || "");
        if (!["editable_layers", "screen_print", "embroidery_prep"].includes(outputMode)) {
          return json(res, 400, { error: "layerize_output_mode_invalid" });
        }
        const fileName = String(body?.fileName || "artwork").trim();
        if (!fileName || fileName.length > 255 || /[\u0000-\u001f\u007f]/.test(fileName)) {
          return json(res, 400, { error: "layerize_file_name_invalid" });
        }
        ...
        const response = await fetchImpl(`${cfg.internalRuntimeUrl}/internal/layerize/run`, {
          ...
          body: JSON.stringify({
            ownerId: user.id,
            payload: { asset, fileName, outputMode },
          }),
        });
