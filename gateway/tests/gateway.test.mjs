...
/**
 * The six Call-1 panel records, in the shape `cutCallOnePanels` stamps them.
 * Print is trim plus the 5" bleed on both edges, which the gateway re-derives
 * rather than trusts.
 */
function callOnePanelRecords({ ownerId, generationId, masterHash, revisionSequence = 1 }) {
  const geometry = [
    ["driver", 196.9, 50.9], ["passenger", 196.9, 50.9], ["hood", 70, 60],
    ["roof", 90, 70], ["front", 80, 30], ["rear", 80, 30],
  ];
  return geometry.map(([surfaceKey, trimWidthIn, trimHeightIn], index) => {
    const contentHash = (index + 5).toString(16).repeat(64).slice(0, 64);
    return {
      surfaceKey,
      storagePath: `designpro/user_${ownerId}/${generationId}/flat-first/v1`
        + `/revisions/${revisionSequence}/panels/${contentHash}.png`,
      contentHash,
      contentType: "image/png",
      byteSize: 5484625 + index,
      pixelWidth: 3712,
      pixelHeight: 1093,
      trimWidthIn,
      trimHeightIn,
...
/** The workspace RPC answer for a generation that exists and is readable. */
function generationWorkspaceRpcRow({ requestId, generationId, ownerId, views = [] }) {
  return {
    requestId,
    generationId,
    ownerId,
    ...
    designName: "Pro-Tech Automotive",
    ...
    updatedAt: "2026-08-25T21:15:41Z",
    completedAt: "2026-08-25T21:15:41Z",
  };
}
...
test("a job route resolves a generation that has no workflow run yet", async (t) => {
  ...
  const server = createGateway({
    ...
    fetchImpl: async (url) => {
      ...
      if (value.endsWith("/auth/v1/user")) return Response.json({ id: userId });
      if (value.includes("/rest/v1/designpro_workflow_runs")) return Response.json([]);
      // The generation is read through the SECURITY DEFINER workspace RPC. The
      // request table itself is service-role only, so a direct select on it is
      // refused for every caller -- which is exactly the defect this route hit.
      if (value.includes("/rest/v1/rpc/designpro_generation_workspace")) {
        return Response.json({
          requestId: "11111111-2222-4333-8444-55555555002a",
          generationId,
          ownerId: userId,
          state: "queued",
          contractVersion: "designpro.calls-1-7-input.v3",
...
test("artifacts and approved views answer empty, not 404, before the handoff", async (t) => {
  ...
  const server = createGateway({
    ...
    fetchImpl: async (url, init) => {
      ...
      if (value.includes("/rest/v1/designpro_workflow_runs")) return Response.json([]);
      if (value.includes("/rest/v1/rpc/designpro_generation_workspace")) {
        // Honour the generation id in the body, so the not-found case below is
        // a real absence rather than a stub that answers everything. The RPC
        // returns null for a generation the caller may not read, which is the
        // same answer it gives for one that does not exist.
        if (!String(requestBody).includes(generationId)) return Response.json(null);
        return Response.json({
          requestId: "11111111-2222-4333-8444-55555555002b",
          generationId,
          ownerId: userId,
          state: "leased",
          contractVersion: "designpro.calls-1-7-input.v3",
