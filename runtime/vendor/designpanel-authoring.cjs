...
// designpanel-authoring source sha256: 0da8dfd041705437dc2b55722afeefc4cfbb33983263f5a8a5415c6ca7e7fb62
...
// ../../../tmp/dpag-authoring-lyIYvZ/authoring.ts
var authoring_exports = {};
__export(authoring_exports, {
  ATLAS_LOGO_AUTHORING_RULE: () => ATLAS_LOGO_AUTHORING_RULE,
  COMMERCIAL_DEPTH: () => COMMERCIAL_DEPTH,
  COMMERCIAL_PERSONA: () => COMMERCIAL_PERSONA,
  COMMERCIAL_TRANSLATION: () => COMMERCIAL_TRANSLATION,
  LOGO_REQUIREMENT: () => LOGO_REQUIREMENT,
  PHOTO_REALISM_LOCK: () => PHOTO_REALISM_LOCK,
  ...
  buildLogoArchitecture: () => buildLogoArchitecture,
  getCameraAngle: () => getCameraAngle,
  splitStyleAndText: () => splitStyleAndText,
  truckBedClause: () => truckBedClause
});
module.exports = __toCommonJS(authoring_exports);

// ../../../tmp/dpag-authoring-lyIYvZ/studio-os.ts
...
// ../../../tmp/dpag-authoring-lyIYvZ/view-angles-os.ts
var CAMERA_ANGLES = {
  side: `
...
// ../../../tmp/dpag-authoring-lyIYvZ/render-events-slice.ts
...
var ATLAS_LOGO_AUTHORING_RULE = "Design an actual brand mark for it \u2014 the company name set in a typeface is not a logo, however well it is set. The name may lock up with the mark, sit beside it or be built into it; the mark's form, register and construction are your call and the brief's.";
...
function buildDesignIQPrompt(params) {
  ...
  if (mode === "artboard") {
    ...
    if (params.atlasTopology === true) {
      const authoringMode = params.authoringMode === "restyle" ? "restyle" : "commercial";
      const atlasQualityExampleCount = Number(params.artboardQualityExampleCount) || 0;
      const atlasWebsite = String(params.website || "").trim();
      const atlasLogoSupplied = params.logoSupplied === true;
...
BRAND: ${companyName} \u2014 integrate the company name + logo + a clean contact bar into the design, legible at a glance.`;
        atlas += atlasLogoSupplied ? ` Spell the business name exactly. The attached verified customer-owned logo is the logo authority; preserve its form, spelling, proportions and palette exactly and never invent a substitute.` : `${buildLogoArchitecture(companyName, industryType)} ${ATLAS_LOGO_AUTHORING_RULE}`;
...
No website was supplied \u2014 invent no website, email address or street address, and display none anywhere on the design.`;
      ...
      const atlasTextLayer = String(params.textLayerPrompt || "").trim();
...
Typography preference: ${fontStyle}.`;
      const atlasKeywords = bulletPoints?.filter((b) => b?.trim());
      if (atlasKeywords?.length) atlas += `
Brand keywords (guide tone \u2014 not literal on-vehicle text): ${atlasKeywords.map((k) => k.trim()).join(", ")}`;
      if (mascot) {
        atlas += `
...
QR CODE ZONE: Reserve one clean, flat, evenly-lit rectangular area (roughly 10x10 inches) low on the rear quarter panel \u2014 free of graphics, text, and busy color \u2014 as space for a scannable QR code added in production. Do not draw a QR code yourself.`;
        const atlasQrUrl = String(params.qrUrl || "").trim();
...
Gallery-grade custom artwork with real depth, movement, and a wow factor \u2014 never generic AI filler, never a template.` + (atlasQualityExampleCount > 0 ? ` Match the production quality of the provided gold-standard DesignPanel artboards.` : ``) + ` Output ONE flat 2D artboard sheet filled with the branded wrap design, drawn straight-on and flat for printing.`;
      return atlas;
    }
...
Gallery-grade custom artwork with real depth, movement, and a wow factor \u2014 never generic AI filler, never a template. Match the layout, labeling, and production quality of the example artboards. Output ONE flat 2D artboard sheet showing the labeled rectangular print panels ${abClean ? "filled with the BACKGROUND DESIGN ONLY \u2014 no text, no logos" : "filled with the branded wrap design"}, drawn straight-on and flat for printing.`;
  ...
  if (mode === "commercial") {
    ...
    let assembled2 = `${COMMERCIAL_PERSONA}
