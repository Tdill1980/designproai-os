...
async function executeProduction(sb, stage, run, runtimeConfig) {
  ...
  if (stage.stage_key === "source.verify") {
    ...
    // Call 11 also publishes `logo` artifacts — the brand-lift Logo Pack cropped
    // from the branded duplicates — and those are placement-free by design:
    // they were detected, not attested, so holding them against the frozen
    // inventory would fail every A.C.E.-designed brand at source.verify.
