/**
 * GENIE SIZE-BAND FIT -- ported from restylepro-os
 * supabase/functions/generate-2d-proof/proof-band-fit.ts.
 * Only the TypeScript annotations and the module system changed.
 *
 ...
 */
function fitBandFontSize(lines, availableWidth) {
 ...
 */
function bandHeight(lines, fontSize) {
  const live = (lines || []).map((l) => String(l || "").trim()).filter(Boolean);
 ...
 */
function estimateBandLineWidth(line, fontSize) {
  return String(line || "").trim().length * fontSize * BAND_AVG_ADVANCE;
}

module.exports = { BAND_AVG_ADVANCE, BAND_LINE_GAP, BAND_MIN_FONT, BAND_MAX_FONT, fitBandFontSize, bandHeight, estimateBandLineWidth };
