 ...
 *
 * ONE DELIBERATE OMISSION: `substrateContext` (index.ts:408). It reads a
 * `substrate` field that does not exist in this runtime's request contract, so
 * porting it would mean inventing an input rather than carrying one across.
 */
function buildRestylePrompt({
  prompt, finish, finishSpec, vehicle, cameraAngle, viewType, wantsPhoto,
  visionBoardImages, visionboardIntent, styleDescriptors,
}) {
...
function buildDesignIQPrompt({
  ...
  visionBoardImages, visionboardIntent, viewType, styleDescriptors, mode,
}) {
