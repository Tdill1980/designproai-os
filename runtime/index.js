...
app.post("/internal/layerize/run", authMiddleware, async (req, res) => {
  try {
    ...
    const result = await layerize.run({
      ownerId,
      asset: payload.asset,
    ...
    const status = Number(error?.status) || (/layerize_tokens_required/.test(String(error?.message || "")) ? 402 : 400);
    console.error(`[LAYERIZE] ${String(error?.message || error)}`);
    return res.status(status).json({
      error: String(error?.message || "layerize_failed").replace(/[^a-z0-9_:-]/gi, "_").slice(0, 120),
    });
  }
});
