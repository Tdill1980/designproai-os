...
function outputClassPrompt(inspectionId) {
  return [
    ...
    "",
    "ALSO vehicle_depiction: a rectangle whose artwork is a vehicle-shaped island — the artwork stops at a die-cut outline and a plain single-colour surround (grey, white, black or any colour) fills the rest of the rectangle. The outline may be a whole vehicle (side profile, front or rear elevation) OR a single body panel's trimmed shape: a hood, roof, door, bed side, tailgate, fender or bumper silhouette. A sheet of such shapes arranged on a plain field is a layout drawing of a wrap, not the printed wrap, and it is vehicle_depiction EVEN WHEN each shape is filled with correct artwork and EVEN WHEN the shapes are captioned. ALSO vehicle_depiction: a rectangle of otherwise continuous artwork that carries a dark or empty OPENING where a wheel, wheel arch, window, windshield or grille would sit — a disc, arch or pane of black, dark grey or blank inside the artwork. Printed vinyl has no openings; the installer cuts them. Small dark graphic details, shadows and lettering are artwork, not openings.",
    "",
