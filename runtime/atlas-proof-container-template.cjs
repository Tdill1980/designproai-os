...
function containerSvg({ manifest = {}, companyName = "", vehicle = "", bleedInches = 5,
  ...
  // VERSION DEFAULTS TO 1.0. The owner's structural specification names the value
  // ("Version (1.0)"), so leaving a ruled blank there is a field the sheet
  ...
  [["DATE:", job.date], ["ORDER #:", job.order], ["DESIGNER:", job.designer],
    ["VERSION:", job.version || "1.0"]].forEach(([k, v], i) => {
