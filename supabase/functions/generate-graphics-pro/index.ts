 ...
 *
 * Recovered from restylepro-os (RULE 1: recover before you invent). The
 * DesignProAI OS deltas are marked inline: the storage bucket is the public
 * `graphicspro-files` bucket this project owns (wrap-files is private here),
 * the production PDF is built by cut-contour-build, and the RestylePro
 * production_flow_assets vault write is dropped.
 *
 * Actions:
 ...
 *   - "lookup_dimensions" → Gemini + Google Search vehicle panel dimensions
 *   - "create_logo_job"   → logo utility job (skips the mockup)
 *   - "run_production"    → Topaz upscale → CUT-MAP™ → cut files → CutContour PDF → pricing
 */
...
async function uploadImage(
  ...
  const { data: { publicUrl } } = supabase.storage.from("graphicspro-files").getPublicUrl(fileName);
  return publicUrl;
}
  ...
  "totalSqFt": <total wrap coverage both sides + hood + roof + rear in square feet>
    ...
    if (action === "run_production") {
      ...
      try {
        ...
        try {
          ...
          if (imgResp.ok) {
            ...
            if (upscaleResult.upscaled) {
              ...
              if (!upErr) {
                const { data: { publicUrl } } = supabase.storage.from("graphicspro-files").getPublicUrl(upPath);
        ...
        // The RestylePro "Build Assets parity" write into production_flow_assets
        // is deliberately NOT carried over: that table is a RestylePro vault this
        // operating system does not have (see App.tsx: designpanelpro-workspace).
        // The graphics_pro_jobs row IS the record of a GraphicsPro job here.
        const parityWarn: string | null = null;
