...
const PRINTER_EMAIL = Deno.env.get("WPW_FULFILLMENT_EMAIL") || "lance@weprintwraps.com";
// The sending identity. ORDER_FROM_EMAIL must be on a domain verified in
// this project's Resend account or Resend refuses the send (the webhook logs
// it and the order still lands as 'paid' for the admin board).
const FROM_EMAIL = Deno.env.get("ORDER_FROM_EMAIL") || "orders@designproai.com";
const BRAND_NAME = Deno.env.get("PATTERNPRO_BRAND_NAME") || "PatternPro";
const PRINTER_NAME = "WePrintWraps";
...
function buildPrinterHtml(p: OrderPayload, orderId: string): string {
  const isPrint = p.orderType === "printed_wrap";
  const isFulfillment = p.mode === "fulfillment";
  const title = isFulfillment
    ? (isPrint ? "🚀 PAID ORDER — PRINT & SHIP" : "🚀 PAID ORDER — Design File")
    : (isPrint ? "New Printed Wrap Quote" : "New Design File Quote");

  const fulfilled = isFulfillment
    ? `Customer has paid through ${BRAND_NAME}. Print & ship to the address below.`
  ...
  // On this deployment WePrintWraps prints under its own name — no white-label
  // "ship blind" banner. What the printer needs is the address and the art.
  const whiteLabelBanner = isFulfillment && isPrint ? `
    <div style="background:#ecfdf5;border:2px solid #10b981;border-radius:8px;padding:12px 16px;margin:16px 0;font-size:14px;color:#065f46;">
      <strong>Paid in full.</strong> Print ${p.yards ?? ""} yards of ${p.pattern.name} (${p.finish}) and ship to the customer below. Send tracking to the customer's email.
    </div>` : "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const subject = isFulfillment
      ? (isPrint
          ? `🚀 PAID — Print & Ship — ${payload.pattern.name} for ${payload.vehicle.label} — $${payload.totalPrice.toFixed(2)}`
    ...
    const html = buildPrinterHtml(payload, orderId);

    const printerResult = await resend.emails.send({
    ...
    if ((printerResult as any).error) {
      console.error("Resend error (printer):", (printerResult as any).error);
      throw new Error(`Failed to send order email: ${(printerResult as any).error.message || "unknown"}`);
    }
    ...
    return new Response(JSON.stringify({ success: true, orderId, sentTo: PRINTER_EMAIL, mode: payload.mode || "quote" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
