...
//
// The durable fix is to move composition to the Railway worker, where there is
...
export async function composeProofSheet(opts: {
  ...
  // ── COMPOSITION RUNS ON THE RAILWAY WORKER ────────────────────────────────
  //
  ...
  const WORKER_SECRET = Deno.env.get("WORKER_SECRET") || Deno.env.get("RENDER_WORKER_KEY") || "";
  ...
  const resp = await fetch(`${WORKER_URL.replace(/\/+$/, "")}/compose-proof-sheet`, {
    method: "POST",
    headers: {
