...
export interface VehicleProofTemplate {
  /** Closed outline the artwork is clipped to, in 0..1 tile space. */
  silhouette: string;
  /** Strokes drawn over the artwork: glass, arches, seams. Never filled. */
  details: string[];
  /** True when the shape is a plain rectangle (trailers, flat panels). */
  rectangular?: boolean;
}

const rect = (): VehicleProofTemplate => ({
 ...
 */
export type BodyFamily = "pickup" | "van" | "car" | "suv" | "box" | "flat";

const FAMILY_BY_TYPE: Record<string, BodyFamily> = {
  truck: "pickup",
  pickup: "pickup",
  van: "van",
  car: "car",
...
export function bodyFamilyFor(vehicleType: unknown): BodyFamily {
 ...
 */
const SIDE: Record<BodyFamily, VehicleProofTemplate> = {
 ...
 */
const PLAN: Record<ProofViewKey, VehicleProofTemplate | null> = {
  "DRIVER SIDE": null,
  "PASSENGER SIDE": null,
  ROOF: {
    silhouette:
 ...
 */
export function vehicleProofTemplate(
  vehicleType: unknown,
  view: string,
): VehicleProofTemplate & { mirrored: boolean } {
  const key = String(view || "").trim().toUpperCase() as ProofViewKey;
  const family = bodyFamilyFor(vehicleType);

  if (key === "DRIVER SIDE" || key === "PASSENGER SIDE") {
    const side = SIDE[family] || rect();
    return { ...side, mirrored: key === "PASSENGER SIDE" };
  }
  ...
  if (family === "flat") return { ...rect(), mirrored: false };

  const plan = PLAN[key];
  return plan ? { ...plan, mirrored: false } : { ...rect(), mirrored: false };
}
 ...
 */
export function vehicleProofSvg(
  template: VehicleProofTemplate & { mirrored: boolean },
  clipId: string,
  x: number,
  y: number,
  w: number,
  h: number,
): { clipPath: string; overlay: string[]; rectangular: boolean } {
  const map = (path: string) =>
    path.replace(
      ...
      (
        ...
        const toX = (v: number) =>
          (template.mirrored ? x + w - v * w : x + v * w).toFixed(2);
        const toY = (v: number) => (y + v * h).toFixed(2);
  ...
  const clipPath =
    `<clipPath id="${clipId}"><path d="${map(template.silhouette)} Z"/></clipPath>`;
  const overlay = [
  ...
  return { clipPath, overlay, rectangular: Boolean(template.rectangular) };
}
