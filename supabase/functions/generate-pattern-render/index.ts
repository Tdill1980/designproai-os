...
/**
 * Where PatternPro's on-vehicle renders live on THIS project. RestylePro wrote
 * them under its public `wrap-files` bucket; here `wrap-files` is PRIVATE (it
 * holds production packs and provider-private atlases), so a public URL into
 * it 400s and every render would be a broken image. PatternPro gets its own
 * public bucket, the same shape GraphicsPro uses (`graphicspro-files`), created
 * by supabase/migrations/20260915090000_patternpro_wbty_products.sql.
 */
const RENDER_BUCKET = "patternpro-files";
...
async function processGeminiResponse(
  ...
  const renderUrl = `${SUPABASE_URL}/storage/v1/object/public/${RENDER_BUCKET}/${storagePath}`;
  ...
  const { data: vizRow, error: vizErr } = await sb
    ...
    .insert({
      ...
      // The client sends the vehicle at the TOP of the body (vehicleYear /
      // vehicleMake / vehicleModel), not inside colorData. Reading only
      // colorData left vehicle_year null on every PatternPro render, the
      // NOT NULL constraint rejected the row, renderId came back null, and
      // nothing reached My Renders / RevisionStudio / the 2D-proof phase.
      // Live-confirmed 2026-09-15 (13 of 13 renders: "DB insert warning 23502").
      vehicle_year: vehicleYear || colorData.vehicleYear || null,
      vehicle_make: vehicleMake || colorData.vehicleMake || vehicle.split(" ")[1] || null,
      vehicle_model: vehicleModel || colorData.vehicleModel || vehicle.split(" ")[2] || null,
