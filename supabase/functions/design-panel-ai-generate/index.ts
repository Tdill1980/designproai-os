...
// The A.T.L.A.S. proof presentation branch renders on the SAME pinned GA image
// model as Call 1 (model-config's PRIMARY_IMAGE_MODEL). FALLBACK_IMAGE_MODEL is
// deliberately not imported: this path does not drop to Flash.
import { geminiImageUrl, PRIMARY_IMAGE_MODEL } from "../_shared/model-config.ts";
...
// ---------------------------------------------------------------------------
// handleAtlasProofPresentation — THE RESTORED LEGACY 3D PRESENTATION PATH.
//
// Owner directive, 2026-09-01 (Restoration Contract, Step 1): restore the
// proven legacy DesignPanelAI presentation intelligence for the 3D proofs,
// without breaking, replacing, weakening or bypassing A.T.L.A.S. This handler
// is that path, and it is STRUCTURALLY ISOLATED per the owner's safety
// amendment: the `mode: "atlas-proof"` discriminator returns before a single
// line of creative assembly runs, and nothing in here is reachable from an
// authoring request.
//
// ZERO ARTWORK AUTHORITY. The canonical Call-1 panel that arrives as a storage
// path + sha256 IS the wrap. This handler chooses the camera, the studio and
// the finish; it never chooses the design. Every prompt byte comes from
// _shared/atlas-proof-presentation.ts.
//
// It does NOT replace persona-photographer-render. That function keeps every
// other shot while Driver is validated against its canonical panel; only the
// runtime decides which producer a given shot goes to.
//
// Differences from the photographer path, both owner-specified:
//   * temperature 1.0 — the legacy stack pinned it and the photographer sets
//     none at all, so it is stated rather than inherited from a model default.
//   * NO Flash fallback — the photographer drops to
//     FALLBACK_IMAGE_MODEL on its last attempt. A proof rendered on a
//     different model than the one this product is judged on is not evidence,
//     and this restoration is being measured. Retries stay on the GA id.
// ---------------------------------------------------------------------------

const ATLAS_PROOF_PRESENTATION_SOURCE_COMMIT = "113d137dbe8813ca3bf70c8d7265ad081ebd4524";

async function handleAtlasProofPresentation(body: Record<string, unknown>): Promise<Response> {
  ...
  const fail = (message: string, status = 400) => new Response(
    JSON.stringify({
      success: false, requestId, functionName: "design-panel-ai-generate",
      contract: ATLAS_PROOF_PRESENTATION_CONTRACT, shotKey, surfaceKey, error: message,
    }),
  ...
  try {
    ...
    // THE PANEL MUST BE THE ONE THIS SHOT IS FOR. A passenger proof built from
    // the driver's artwork looks plausible and is wrong; refuse, never render.
    ...
    // RULE 0.29, behaviour 3: the Driver continuity reference is ruled out by
    // name. Cross-view identity rests on the shared master, hash-bound per
    ...
    // `wrap-files` is private — the panel travels as a STORAGE PATH plus
    // sha256, never a URL, and the bytes are verified to be the artifact the
    // caller named before they can condition anything.
    ...
    if (!vehicle) return fail("atlas_proof_vehicle_missing");
    ...
    const prompt = buildAtlasProofPresentationPrompt({
      ...
      // Vehicle CONFIG, which is A.T.L.A.S.'s to contribute (owner, 2026-08-28:
      // "ATLAS should contribute only: exact panel artwork; lineage; exact
      // vehicle/config; requested shot"). The runtime already sends it.
      isPickup: body.isPickup === true,
    });

    // THE PANEL FIRST, THE CAMERA LAST. Gemini weights the final part most
    // heavily, and the camera instruction is the one that must win.
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        if (!response.ok) {
          console.error(`atlas-proof-presentation ${requestId} ${shotKey}: HTTP ${response.status} (attempt ${attempt})`);
        ...
        console.error(`atlas-proof-presentation ${requestId} ${shotKey}: attempt ${attempt} error`, (err as Error)?.message);
      }
      if (attempt < MAX_ATTEMPTS) await new Promise((r) => setTimeout(r, 2000 * attempt));
    }
    if (!imageBase64) return fail(`atlas_proof_no_image after ${imageRequestCount} request(s)`, 502);

    const proofBytes = decodeBase64(imageBase64);
    ...
    return new Response(
      JSON.stringify({
        ...
        contract: ATLAS_PROOF_PRESENTATION_CONTRACT,
        sourceCommit: ATLAS_PROOF_PRESENTATION_SOURCE_COMMIT,
        provider: "google",
        model: PRIMARY_IMAGE_MODEL,
        functionVersion: ATLAS_PROOF_PRESENTATION_SOURCE_COMMIT,
        imageRequestCount,
    ...
    console.error(`atlas-proof-presentation ${requestId} failed:`, err);
...
serve(async (req) => {
  ...
  try {
    ...
    // ═══ ATLAS-PROOF — THE PRESENTATION BRANCH, STRUCTURALLY ISOLATED.
    //
    // Owner safety amendment, 2026-09-01: the proof path may reuse this edge
    // surface, but the code paths must be structurally isolated. This returns
    // BEFORE the creative destructuring below, exactly as atlas-artboard does,
    // so an authoring request never reaches a line of presentation code and a
    // proof request never reaches a line of creative assembly.
    //
    // Its prompt comes wholly from _shared/atlas-proof-presentation.ts, which
    // is recovered from the proven generate-color-render LIBRARY PANEL branch.
    // No creative construction happens here or there: the canonical A.T.L.A.S.
    // panel is the artwork, and this branch decides only how it is photographed.
    if (body?.mode === "atlas-proof") {
      if (!internalCaller.internal) {
        return new Response(
          JSON.stringify({ success: false, error: "atlas_proof_internal_only" }),
          { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
      return await handleAtlasProofPresentation(body);
    }
    const {
      mode,
      prompt,
