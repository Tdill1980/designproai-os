...
import { buildFadeWrapsMyVehiclePrompt } from "./prompt.ts";

/**
 * FADEWRAPS MyVehiclePro — Edge Function
 *
 * Applies a fade/gradient vinyl wrap to a customer's real vehicle photo.
 * Owned exclusively by FadeWraps. Tune freely without affecting other tools.
 */
...
serve(async (req) => {
  ...
  // Token gate — 1 token per FadeWraps render.
  const gate = await tokenGate(req, { reason: "fadewraps_on_vehicle_photo_render" });
  ...
  try {
    ...
    if (!colorData) {
      return new Response(
        JSON.stringify({ error: "Fade selection required" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    console.log("📸 fadewraps-on-vehicle-photo: starting", {
      userEmail,
    ...
    const aiPrompt = buildFadeWrapsMyVehiclePrompt({
      fadeStyle: colorData.fadeStyle,
      fadeSpec: colorData.fadeSpec,
      colorName: colorData.colorName,
      colorHex: colorData.colorHex || colorData.hex,
    ...
    const storagePath = `renders/${userPrefix}myvehicle/${timestamp}_${vehicleMake}_${vehicleModel}_fadewraps.${fileExt}`;
    ...
    const { data: vizRecord } = await supabase
      ...
      .insert({
        ...
        color_hex: colorData.colorHex || colorData.hex || "#000000",
        color_name: colorData.fadeStyle || colorData.colorName || "Fade Wrap",
    ...
    console.error("🚨 fadewraps-on-vehicle-photo error:", error);
    return new Response(
