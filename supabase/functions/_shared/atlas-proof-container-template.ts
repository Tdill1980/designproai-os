...
export interface ContainerOptions {
  ...
  /** Authoritative GENIE inches for labels; never changes artwork geometry. */
  dimensionManifest?: ContainerManifest;
...
const textWidth = (value: unknown, size: number) =>
  String(value).length * CHAR_ADVANCE * (size / 9.5);
...
export function layoutRow(
  ...
  const { top, height, left = 56, right = 1482, gap = 56 } = opts;
...
function row(
  surfaces: ContainerSurface[],
  opts: { top: number; height: number; fill?: string; detail?: (cell: ContainerCell) => string[] },
): string {
  const { detail } = opts;
 ...
 */
export interface CutGraphicCell {
  surfaceKey: string; x: number; y: number; w: number; h: number;
  caption: string; note: string;
}

export const ZONE3_SLOTS = Object.freeze([
...
export function layoutCutGraphics(): CutGraphicCell[] {
  let x = ZONE3_BAND.left;
  return ZONE3_SLOTS.map((slot) => {
    const cell: CutGraphicCell = {
      surfaceKey: slot.key, x, y: ZONE3_BAND.top, w: slot.w, h: ZONE3_BAND.height,
...
export function containerSvg(options: ContainerOptions = {}): string {
  ...
  const trimOf = (s: ContainerSurface) => {
