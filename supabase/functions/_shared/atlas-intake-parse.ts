/**
 * _shared/atlas-intake-parse.ts — RAW CUSTOMER TEXT → THE STRUCTURED SCHEMA.
 * ═══════════════════════════════════════════════════════════════════════════
 *
 ...
 *
 * BYTE-LOCKED against runtime/atlas-intake-parse.cjs by
 * tests/atlas-intake-parse.test.mjs — two homes for one contract is how a fix
 * here keeps coming undone. Change both or neither.
 *
...
export const INTAKE_CONTRACT = "designpro.atlas-intake-parse.v1";
export const INTAKE_MODEL = "gemini-2.5-flash";
 ...
 */
export const PHONE = /(?:\+?1[\s.-]*)?(\(?\d{3}\)?[\s.-]{1,2}\d{3}[\s.-]\d{4})/;

/** A bare domain, not an email address and not a sentence-ending word. */
export const WEBSITE = /(?<![\w@.])((?:[a-z0-9][a-z0-9-]*\.)+(?:com|net|org|co|io|us|biz|shop|services|company))(?![\w@])/i;

export const YEAR = /\b(19[8-9]\d|20[0-4]\d)\b/;
 ...
 */
export const MAKES = Object.freeze([
...
const CANONICAL_MAKE: Record<string, string> = Object.freeze({ chevy: "Chevrolet", vw: "Volkswagen", mercedes: "Mercedes-Benz" });
...
const titleCase = (value: unknown): string => String(value || "").trim().split(/\s+/)
  .map((word: string) => (/^[0-9]/.test(word) || (word.length <= 3 && word === word.toUpperCase())
    ? word
 ...
 */
export function intakePrompt(text: string): string {
  return [
 ...
 */
export function mergeIntake(deterministic: Record<string, unknown> = {}, parsed: Record<string, unknown> = {}) {
