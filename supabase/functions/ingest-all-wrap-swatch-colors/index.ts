...
async function validateSwatchImage(imageUrl: string): Promise<{ isValid: boolean; score: number }> {
...
async function extractMaterialProfile(imageUrl: string, finish: string, hex: string): Promise<any | null> {
...
async function processSwatch(
  ...
  try {
      ...
      if (images.length > 0) {
        ...
        const validations = await Promise.all(
          toValidate.map(async (img) => {
            ...
            const result = await validateSwatchImage(img.url);
    ...
    if (!material_validated || !lab) {
      material = await retry(() => extractMaterialProfile(bestImage!, finish, hex));
      if (material?.lab) {
  ...
  try {
    ...
    for (let i = 0; i < swatches.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.all(
        batch.map(swatch => processSwatch(swatch, supabase, dataForSeoKey))
      );
