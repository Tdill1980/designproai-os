...
import { mergeOrdersByDate, patternProOrdersFor } from "../_shared/shopflow-patternpro.ts";
import {
  extractTracking,
  WooLineItem,
  WooOrder,
...
/**
 * The customer's live WPLoyalty balance, if the loyalty webhook plugin has
 * ever pushed one for this email (see wpw-loyalty-webhook /
 * wordpress/wpw-loyalty-webhook/). Never computed here — a customer with no
 * row yet (plugin not installed, or no point event since install) gets
 * `null`, and the frontend falls back to linking out to WPLoyalty's own page
 * rather than showing an invented number.
 */
async function fetchLoyaltyPoints(
  sb: ReturnType<typeof createClient>,
  email: string | null,
): Promise<{ points_balance: number; updated_at: string } | null> {
  ...
  const { data } = await sb
    ...
    .maybeSingle();
  return data as { points_balance: number; updated_at: string } | null;
}

/**
 * The customer's own saved quotes, for the "Your quotes" block in ShopFlow.
 *
 * Resolved by the SAME email this function already proves for orders, so a
 * quote is only ever returned to the person whose email is on it. No new
 * identity path, and nothing here is readable without clearing that check.
 *
 * WHY IT MATTERS THAT THESE LIVE IN customer_quotes: wpw-sync-orders matches a
 * paid Woo order back to this table and writes quote_converted_to_order, so a
 * quote shown here becomes measurable revenue the moment the customer orders.
 * That link already works; this is the missing half — the customer could not
 * SEE the quote they were meant to act on.
 *
 * Capped and newest-first. Cancelled quotes are omitted: offering "order this"
 * on a quote someone already killed is worse than not showing it.
 */
async function fetchQuotes(
  ...
  const { data, error } = await sb
    ...
    .select(
      "id,quote_number,created_at,status,quote_total,total_sqft,film_name,film_manufacturer,vehicle_year,vehicle_make,vehicle_model,notes,lead_source",
    )
    .eq("customer_email", email.toLowerCase())
    ...
    .limit(25);
  if (error) {
    // A quote list is an enhancement; never fail an order load over it.
    console.warn("[wpw-orders-read] quote lookup failed:", error.message);
    return [];
  }
  return data || [];
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!forceRefresh && cacheAgeSec < CACHE_TTL_SECONDS) {
      ...
      const cachedEmail =
        (cached?.[0] as { customer_email?: string } | undefined)?.customer_email || null;
      const loyalty = await fetchLoyaltyPoints(sb, cachedEmail);
      const quotes = await fetchQuotes(sb, cachedEmail);
      ...
      return new Response(
        JSON.stringify({
          ...
          linked: true,
          fromCache: true,
          // PatternPro (Stripe, wbty_orders) orders for this signed-in email,
          // in the same list — ShopFlow is the one tracker.
          orders: mergeOrdersByDate(
            (cached || []) as Array<{ date_created?: string | null }>,
            await patternProOrdersFor(sb, authed.email || "") as unknown as Array<{ date_created?: string | null }>,
          ),
          loyalty,
          quotes,
        }),
    ...
    if (wooOrders.length === 0) {
      const loyalty = await fetchLoyaltyPoints(sb, authed.email);
      const quotes = await fetchQuotes(sb, authed.email);
      return new Response(
        JSON.stringify({
          ok: true, linked: true, fromCache: false,
          orders: await patternProOrdersFor(sb, authed.email || ""),
          loyalty,
          quotes,
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    ...
    const hydratedEmail =
      (hydrated?.[0] as { customer_email?: string } | undefined)?.customer_email || authed.email;
    const loyalty = await fetchLoyaltyPoints(sb, hydratedEmail);
    const quotes = await fetchQuotes(sb, hydratedEmail);
    ...
    return new Response(
      JSON.stringify({
        ...
        fromCache: false,
        orders: mergeOrdersByDate(
          (hydrated || []) as Array<{ date_created?: string | null }>,
          await patternProOrdersFor(sb, authed.email || "") as unknown as Array<{ date_created?: string | null }>,
        ),
        loyalty,
        quotes,
