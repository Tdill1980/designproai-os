...
/**
 * Where Stripe sends the customer back. PatternPro is served on more than one
 * host (the DesignProAI shell, and the WePrintWraps partner host that wears
 * it), and a customer who started on wallpro.weprintwraps.com must land back
 * on wallpro.weprintwraps.com — not on a different brand mid-purchase. So the
 * request's Origin wins when it is one of ours; anything else falls back to
 * PUBLIC_SITE_URL.
 */
const ALLOWED_ORIGINS = new Set([
  "https://os.designproai.com",
  "https://wallpro.weprintwraps.com",
  "https://www.designproai.com",
  "https://designproai.com",
]);
const DEFAULT_SITE_URL = Deno.env.get("PUBLIC_SITE_URL") || "https://os.designproai.com";
function siteUrlFor(req: Request): string {
  const origin = (req.headers.get("origin") || "").replace(/\/$/, "");
  return ALLOWED_ORIGINS.has(origin) ? origin : DEFAULT_SITE_URL;
}
const PRICE_PER_YARD_CENTS = 9550; // $95.50 retail
const DESIGN_FILE_PRICE_CENTS = 4900; // $49.00 retail (the platform keeps 100%)
// WPW prints at 98% wholesale; the platform keeps a 2% margin
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${SITE_URL}/pattern-wrap?cancelled=1`,
      ...(isPrint ? {
