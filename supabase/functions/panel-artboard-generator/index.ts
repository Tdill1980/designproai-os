...
serve(async (req) => {
  ...
  try {
    ...
    // Proxies the self-hosted DesignPro worker's /lift-overlays (locate → erase → compose →
    ...
    if (step === "liftoverlays") {
      ...
      if (!DESIGNPRO_WORKER_HOST) return out({ success: false, error: "DESIGNPRO_WORKER_URL is not configured" }, 500);
      ...
      const wr = await fetch(`${DESIGNPRO_WORKER_HOST.replace(/\/+$/, "")}/lift-overlays`, {
        method: "POST",
    ...
    // the self-hosted DesignPro worker; these steps forward to it with the server-side
    ...
    if (step === "sidefield" || step === "passengermirror" || step === "paneljudge") {
      ...
      const r = await fetch(`${DESIGNPRO_WORKER_HOST.replace(/\/+$/, "")}${path}`, {
        method: "POST",
