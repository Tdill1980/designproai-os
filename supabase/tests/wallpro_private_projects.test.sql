...
select lives_ok($$insert into public.wallpro_projects(id,owner_id,name,config) values ('f0606060-6060-4060-8060-606060606060','a0101010-1010-4010-8010-101010101010','Measured wall','{"wallPath":"a0101010-1010-4010-8010-101010101010/uploads/c0303030-3030-4030-8030-303030303030.jpg","width":120,"height":96}')$$,'customer saves their measured wall');
select is((select count(*)::int from public.wallpro_projects where id='f0606060-6060-4060-8060-606060606060'),1,'owner can reopen saved placement');
select lives_ok($$insert into storage.objects(bucket_id,name) values ('wallpro-files','a0101010-1010-4010-8010-101010101010/uploads/c0303030-3030-4030-8030-303030303030.jpg')$$,'owner uploads a wall image in their private namespace');
select is((select count(*)::int from storage.objects where bucket_id='wallpro-files' and name='a0101010-1010-4010-8010-101010101010/uploads/c0303030-3030-4030-8030-303030303030.jpg'),1,'owner can read their uploaded photo');
reset role;

select set_config('request.jwt.claims','{"role":"authenticated","sub":"b0202020-2020-4020-8020-202020202020"}',true);
set local role authenticated;
select is((select count(*)::int from public.wallpro_projects where id='f0606060-6060-4060-8060-606060606060'),0,'another customer cannot read the project');
select is((select count(*)::int from public.wallpro_generations where owner_id='a0101010-1010-4010-8010-101010101010'),0,'another customer cannot read AI history');
select is((select count(*)::int from storage.objects where bucket_id='wallpro-files' and name='a0101010-1010-4010-8010-101010101010/uploads/c0303030-3030-4030-8030-303030303030.jpg'),0,'another customer cannot read the photo');
select throws_ok($$insert into storage.objects(bucket_id,name) values ('wallpro-files','a0101010-1010-4010-8010-101010101010/uploads/d0404040-4040-4040-8040-404040404040.png')$$,'42501',null,'another customer cannot upload into the owner namespace');
select throws_ok($$insert into public.wallpro_projects(owner_id) values ('a0101010-1010-4010-8010-101010101010')$$,'42501',null,'another customer cannot forge project ownership');
reset role;
