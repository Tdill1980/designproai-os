-- A.T.L.A.S. masters must be solid panels, so v4 masters stop qualifying.
--
-- A wrap panel is a solid rectangle. The installer cuts the wheel opening and
-- the window out of the finished panel, which means the printed artwork has to
-- exist in those places for them to cut. The v4 authoring prompt allowed a zone
-- to come back as a vehicle silhouette with the wheel arches and glass punched
-- out and filled flat black -- live evidence 2026-08-23, the Becky's Bakery
-- master -- and every deterministic check reported pass, because a black hole is
-- opaque.
--
-- The runtime now states the solid-panel rule positively and the deterministic
-- master QC measures flat-black blob interiors against the surrounding
-- brightness, so a hole punched in light artwork fails while a genuinely dark
-- design still passes. Those are runtime changes. This migration is the database
-- half: flat_first_atlas_view_set_valid pins the exact authoring prompt version,
-- so it has to learn v5 or every corrected master would be refused here as
-- unrecognised and the owner would lose the read.
--
-- v4 is deliberately NOT kept as an alternative. A master authored under the
-- prompt that produced the holes must not satisfy the current contract, and the
-- version string is exactly the mechanism that refuses it. Accepting both would
-- hand a v4 silhouette straight to Call 9.
--
-- The body below is the 20260822080000 definition reproduced verbatim with that
