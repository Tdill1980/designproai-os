-- A.T.L.A.S. enters the production handoff. It never could.
--
-- RULE 0.17 says the atlas runs the SAME file-output chain as Standard, and
-- 20260823220000 opened `designpro_flat_first_handoff_gate` so it would. But the
-- handoff function itself still carried the older backstop:
--
--   IF v_input_contract NOT IN ('...v1','...v2') THEN
--     RAISE EXCEPTION 'generation_contract_not_production_eligible';
--
-- so the gate opened onto a closed door. Every v3 request raised on the very
-- next statement, which is why production shows all three atlas masters with
-- zero workflow runs, zero revision sources carrying callOnePanels, and a
-- panels.build promotion path that has never once executed. The panels, the
-- master, the seven proofs -- all produced, none able to reach manufacturing.
--
-- 20260824010000 taught callOnePanels to cross the seam and re-stated this
-- refusal in the same breath, which is the wholesale-redefinition hazard its own
-- Close-Up guard warns about, landing on the atlas admission instead.
--
-- This is the 20260824010000 body with three changes and nothing else:
--   * v3 is admitted alongside v1/v2;
--   * the snapshot records the contract it actually came from, instead of
--     hardcoding v2 onto a v3 run;
--   * the direct-insert branch (which exists because generic revision ingestion
--     rejects an unbound source) covers v3 too, since a v3 run has no WrapBox
--     recipient binding either.
--
-- Unchanged: the owner/state/handoff-state proofs, the seven view identities,
-- the Close-Up carry and its guard, the logo placement refusal, the callOnePanels
-- read, idempotency, and the entice workflow creation.
...
BEGIN
  ...
  ELSE
    ...
    v_snapshot:=pg_catalog.jsonb_build_object(
      'contractVersion','designpro.revision-snapshot.v1',
      'sourceInputContract',v_input_contract,
      'revisionId',v_revision,
      'generationId',v_row.generation_id,
      'visualizationId',v_row.id,
      'designId','DID-'||pg_catalog.upper(pg_catalog.left(
...
-- The atlas must stay admitted. A later wholesale redefinition that restores the
-- v1/v2-only backstop puts the pipeline back where it was on 2026-08-25: gate
-- open, door shut, and no way to tell from the gate alone.
DO $migration$
DECLARE
  v_definition text;
BEGIN
  ...
  IF pg_catalog.strpos(v_definition,'designpro.calls-1-7-input.v3')=0 THEN
    RAISE EXCEPTION 'designpro_handoff_refuses_atlas';
  END IF;
END
$migration$;
