...
BEGIN
  ...
  -- The retired producer must be GONE, not merely outvoted -- and GONE means
  -- absent from the whole definition, comments included, because a comment is
  -- part of prosrc and a future reader greps the body, not this file. The
  -- shadow apply caught the first draft naming the retired stage inside the
  -- replacement comment and refused it, which is the check doing its job.
