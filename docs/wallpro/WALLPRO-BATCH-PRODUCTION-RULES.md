...
Panel output today: 51-inch printable width, 1-inch bleed, 0.5-inch overlap by default (`wallpro-print-plan.ts`). The rules ask for media width and printable width stored separately with a 1-inch duplicated overlap; that is a print-plan setting change, tracked separately from the catalog.
