...
Panel output today: 59-inch printable width (the roll is 59 in; the half-inch overlap is inside it) (`WALLPRO_PRINT_WIDTH`, owner 2026-09-11), 1-inch bleed, 0.5-inch overlap by default (`wallpro-print-plan.ts`). The rules ask for media width and printable width stored separately with a 1-inch duplicated overlap; that is a print-plan setting change, tracked separately from the catalog.
