...
### Resolution — RULED 2026-09-23: the template, never a second producer

The per-panel resolution pass built on 09-23 is **deleted**. It re-authored each
Zone 1 panel on its own blank canvas outside the colour-coded template, which
made a second producer of the panels the proof is supposed to be the only source
of. Owner: *"we must use the code built template that's already zoned tricolor"*
/ *"delete the bad one so we can't make this mistake ever."*
Guarded by `tests/zone1-comes-only-from-the-template.test.mjs`.

- [x] **The ceiling is measured, from `layoutRow` on a real manifest.** Zone 1 is
      a 150px band, 1146px usable, six ways: driver cell 272 × 107, every surface
      within 0.04 of **5.06 px/in**.
- [x] **The delivered sheet is ~5056px wide**, so one panel across the whole
