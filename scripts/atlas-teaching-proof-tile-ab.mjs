#!/usr/bin/env node
/**
 * TEST 7 — is the teaching proof's MORPHOLOGY the conditioning signal?
 *
 * Six isolated variables have been tested and all six were null: the proof's
 * field colour, the blank guide, the normalized topology text, the centre order,
 * one clause of the output contract, and the printed labels. Every draw of all
 * six scored 0 compliant zones.
 *
 * None of them touched what the example actually LOOKS like. The owner proof
 * shows six panels FLOATING IN A SEPARATION FIELD — which is precisely what the
 * outputs return: rectangles drawn smaller than the region they were given,
 * sitting in a field. Test 5 measured that directly (panels rectangular, 76.5%
 * of the zone not artwork). The example demonstrates "six panels placed on a
 * sheet"; the contract wants "these six regions ARE the printable artwork".
 *
 *   A  the current owner-approved teaching proof
 *   B  the same six panels scaled to TILE the canvas edge to edge, no field
 *
 * No panel is cropped, reordered or recoloured — only scaled. All artwork and
 * Flamingo branding is preserved. Creative prompt, model, config, aspect ratio,
 * guide state, topology state, output-class wording, manifest, GENIE, extraction
 * and thresholds are untouched, and parts 0 and 1 are sha-identical between arms.
 *
 * TILING ALSO REMOVES THE SIX TECHNICAL LABELS, because they live in the gutters
 * it closes. That is a variable with a MEASURED NULL rather than an unexamined
 * confound: test 6 (run 33589628761) erased the labels on their own and arm B
 * still printed labels — including "A.T.L.A.S. ARTBOARD", a string that appears
 * nowhere in the image — proving the leak is textual.
 *
 * PRIMARY ENDPOINT is the A.T.L.A.S. contract, not the output class: SIX
 * CONTINUOUS RECTANGULAR ARTWORK REGIONS FILLED EDGE TO EDGE TO THEIR
...
import { buildTiledTeachingProof } from "./atlas-teaching-proof-tile.mjs";
...
/**
 * The two requests, and the guards that keep the tiled proof the ONLY
 ...
 */
export function buildTileRequests({
  prompt, teachingReferenceText, teachingBytes, tiledBytes, model,
  ...
  const partsB = [...head, image(tiledBytes)];
  ...
  const requests = { A: describe("A-owner-teaching-proof", partsA), B: describe("B-tiled-teaching-proof", partsB) };
...
async function main() {
  ...
  const tiled = await buildTiledTeachingProof(teachingBytes);
  writeFileSync(join(OUT, "teaching-proof-A-owner.png"), teachingBytes);
  writeFileSync(join(OUT, "teaching-proof-B-tiled.png"), tiled.bytes);
  log(`tile: six panels scaled to cover the canvas — ${tiled.report.tiles.map((t) => `${t.key} ${t.from.w}x${t.from.h}->${t.to.w}x${t.to.h}`).join(", ")}`);
  log(`      canvas fully covered, 0 overlapping tiles, ${tiled.report.residualFieldPixels} residual field pixels `
    + `(${(tiled.report.residualFieldShare * 100).toFixed(3)}%)`);

  const { partsA, partsB, requests, serialize } = buildTileRequests({
    prompt: assembled.prompt,
    teachingReferenceText: call1.TEACHING_REFERENCE_TEXT,
    teachingBytes,
    tiledBytes: tiled.bytes,
  ...
  writeFileSync(join(OUT, "tile-report.json"), JSON.stringify(tiled.report, null, 2));
  writeFileSync(join(OUT, "requests.json"), JSON.stringify({ vehicle: VEHICLE, brief: BRIEF, draws: DRAWS, tile: tiled.report, requests }, null, 2));

  if (captureOnly) {
    log("capture-only: both requests written, no provider call made");
    writeFileSync(join(OUT, "results.json"), JSON.stringify({ captureOnly: true, imageRequestsExecuted: 0, parity, tile: tiled.report, requests }, null, 2));
    return;
  }
  ...
  for (let n = 1; n <= DRAWS; n += 1) {
    for (const [arm, parts] of [["A", partsA], ["B", partsB]]) {
      const label = `${arm}${n}`;
      const file = `${label}-${arm === "A" ? "owner-proof" : "tiled-proof"}.png`;
      log("");
  ...
  const results = {
    contract: "designpro.call1-teaching-proof-tile.v1",
  ...
  writeFileSync(join(OUT, "COMPARISON.md"), [
    "# Test 7 — teaching-proof panels tiled edge to edge, raw pre-repair masters",
    "",
    "A = the owner teaching proof (six panels floating in a separation field).",
    "B = the same six panels scaled to TILE the canvas edge to edge, no field.",
    `Only scaled: no crop, no reorder, no recolour. ${DRAWS} draws per arm, interleaved.`,
    "",
...
if (process.argv[1] && process.argv[1].endsWith("atlas-teaching-proof-tile-ab.mjs")) {
  main().catch((error) => {
