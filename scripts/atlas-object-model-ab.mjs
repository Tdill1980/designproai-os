#!/usr/bin/env node
/**
 * TEST 7 — is Gemini's mental model of the OUTPUT OBJECT wrong?
 *
 * Six image-side variables were null. The hypothesis is no longer "wheel wells".
 * It is that Gemini is being told to DRAW SIX VEHICLE-PANEL OBJECTS ON AN
 * ARTBOARD when the contract wants it to FILL SIX AUTHORITATIVE RECTANGULAR
 * PRINT-MEDIA REGIONS EDGE TO EDGE. One wrong object model explains the vehicle
 * contours, the wheel wells, the rectangles drawn smaller than their zones, the
 * surrounding empty fields, the copied technical labels and the invented
 * "A.T.L.A.S. ARTBOARD" string simultaneously.
 *
 *   A  the exact deployed Call-1 request
 *   B  identical, except the model-facing OUTPUT/OBJECT tail is reframed
 *
 * The deployed prompt is [DesignPanelAI creative assembly] + [flat-master output
 * contract], and the contract is the TAIL. B keeps the creative assembly byte
 * for byte and swaps only that tail, so "the creative intelligence is preserved"
 * is structural rather than asserted. Teaching proof, guide, topology state,
 * model, config, 1:1/4K, GENIE, manifest, extraction and every threshold are
 * untouched, and the surface-name list is KEPT for this screening test --
 * removing it would be a second variable.
 *
 * NO NEGATIVE IS ADDED, and B carries none: the contract builder refuses to emit
 * a tail containing wheel/window/glass/silhouette/hole/arch/seam/bumper/
 * headlight/artboard/template/"do not"/"avoid".
 *
 * TWO DRAWS PER ARM. A screening test, not a repeatability test: four calls are
 * enough to see whether redefining the object produces a major signal. If it
 * does, repeatability is established afterwards, not assumed here.
 *
 ...
 *   PRINTABLE ARTWORK.
 * Per pane: artwork reaches all four authoritative boundaries; no floating
 * smaller rectangle; no surrounding field; no vehicle-shaped contour or opening;
 * no technical furniture. `flat_atlas` by itself is NOT a pass.
 *
...
import { buildPrintMediaPrompt } from "./atlas-print-media-contract.mjs";
...
/**
 * The two requests, and the guards that keep the output-contract tail the ONLY
 * change. Exported so a test can convict the creative assembly drifting, a part
 * other than the prompt moving, or a negative appearing in B.
 */
export function buildObjectModelRequests({
  prompt, teachingReferenceText, targetGuideText, teachingBytes, guideBytes, model,
  expectedTeachingSha256 = TEACHING_PROOF_SHA256,
}) {
  const reframed = buildPrintMediaPrompt(prompt);
  if (!prompt.startsWith(reframed.creative)) throw new Error("the creative assembly is not a prefix of the deployed prompt");
  if (!reframed.prompt.startsWith(reframed.creative)) throw new Error("the creative assembly did not survive the swap");
  if (reframed.prompt === prompt) throw new Error("the prompt is unchanged — nothing is being tested");
  ...
  const partsB = [{ text: reframed.prompt }, ...tail];
  ...
  const requests = {
    ...
    B: describe("B-print-media-object", partsB, reframed.prompt),
  };
  ...
  for (let i = 1; i < 5; i += 1) {
    if (requests.A.parts[i].sha256 !== requests.B.parts[i].sha256) {
      throw new Error(`part ${i} differs between arms — only the prompt tail may change`);
    }
  }
...
async function main() {
  ...
  const { partsA, partsB, requests, serialize, reframed } = buildObjectModelRequests({
  ...
  log(`  creative assembly ${reframed.creative.length} chars, byte-identical in both arms`);
  log(`  A tail ${reframed.deployedTail.length} chars, sha ${sha(reframed.deployedTail).slice(0, 16)}`);
  log(`  B tail ${reframed.printMediaTail.length} chars, sha ${sha(reframed.printMediaTail).slice(0, 16)}`);
  log(`  A prompt sha ${requests.A.promptSha256.slice(0, 16)}   B prompt sha ${requests.B.promptSha256.slice(0, 16)}`);
  writeFileSync(join(OUT, "prompt-A.txt"), assembled.prompt);
  writeFileSync(join(OUT, "prompt-B.txt"), reframed.prompt);
  writeFileSync(join(OUT, "tail-A.txt"), reframed.deployedTail);
  writeFileSync(join(OUT, "tail-B.txt"), reframed.printMediaTail);
  ...
  const results = {
    contract: "designpro.call1-object-model.v1",
...
if (process.argv[1] && process.argv[1].endsWith("atlas-object-model-ab.mjs")) {
  main().catch((error) => {
