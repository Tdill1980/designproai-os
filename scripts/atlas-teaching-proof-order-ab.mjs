#!/usr/bin/env node
/**
 * TEST 4 — is the teaching proof's own panel ORDER a causal conditioning
 * problem?
 *
 * The owner-approved proof's centre column reads HOOD → ROOF → REAR → FRONT
 * from its own printed labels. `CENTER_ORDER`, `manifest.zones`, the normalized
 * topology and the deterministic panel cut all read REAR → ROOF → HOOD → FRONT.
 * HOOD and REAR are swapped between the example Call 1 learns from and the
 * geometry Call 1 is cut against. Test 3 put both in one request and the output
 * class collapsed to `vehicle_depiction` on all three draws, which made that
 * test inconclusive rather than a verdict on topology.
 *
 *   A  the current owner teaching proof
 *   B  the SAME proof with only the HOOD and REAR blocks swapped, so its centre
 *      column reads REAR → ROOF → HOOD → FRONT
 *
 * Guide absent in both. No topology text. Creative prompt, model, config, image
 * order and customer fixture identical. Three draws per arm, interleaved.
 *
 * PRIMARY ENDPOINT: correct flat A.T.L.A.S. output-class stability.
 * SECONDARY: six-rectangle compliance (colour-blind), non-artwork share, contour
 * score, Driver/Passenger cohesion, black-hole ratio, latency.
 *
 * The production teaching proof is never written and its edge-function hash pin
 * is untouched. Nothing else in production changes either.
 */
import { createHash } from "node:crypto";
import { mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { fullBleedMetrics } from "./atlas-fullbleed-metrics.mjs";
import { buildReorderedTeachingProof } from "./atlas-teaching-proof-reorder.mjs";
...
/**
 * The two requests, and the guards that keep the reordered proof the ONLY
 ...
 */
export function buildReorderRequests({
  prompt, teachingReferenceText, teachingBytes, reorderedBytes, model,
  ...
  const partsB = [...head, image(reorderedBytes)];
  ...
  const requests = { A: describe("A-owner-teaching-proof", partsA), B: describe("B-reordered-teaching-proof", partsB) };
...
async function main() {
  ...
  const reordered = await buildReorderedTeachingProof(teachingBytes);
  writeFileSync(join(OUT, "teaching-proof-A-owner.png"), teachingBytes);
  writeFileSync(join(OUT, "teaching-proof-B-reordered.png"), reordered.bytes);
  log(`reorder: HOOD block ${reordered.report.hoodBlock.height}px -> y${reordered.report.hoodBlock.movedTo}, `
    + `REAR block ${reordered.report.rearBlock.height}px -> y${reordered.report.rearBlock.movedTo}; `
    + `${reordered.report.changedPixels} pixels changed, flanks/ROOF/FRONT unchanged, `
    + `centre order ${reordered.report.centerOrderBefore.join(">")} -> ${reordered.report.centerOrderAfter.join(">")}`);

  const { partsA, partsB, requests, serialize } = buildReorderRequests({
    prompt: assembled.prompt,
    teachingReferenceText: call1.TEACHING_REFERENCE_TEXT,
    teachingBytes,
    reorderedBytes: reordered.bytes,
  ...
  log(`arm B swaps only the teaching image — ${requests.B.modelRequestByteSize} bytes, ${requests.B.partCount} parts, 1 image, sha ${requests.B.parts[2].sha256.slice(0, 16)}`);
  writeFileSync(join(OUT, "reorder-report.json"), JSON.stringify(reordered.report, null, 2));
  writeFileSync(join(OUT, "requests.json"), JSON.stringify({ vehicle: VEHICLE, brief: BRIEF, draws: DRAWS, reorder: reordered.report, requests }, null, 2));

  if (captureOnly) {
    log("capture-only: both requests written, no provider call made");
    writeFileSync(join(OUT, "results.json"), JSON.stringify({ captureOnly: true, imageRequestsExecuted: 0, parity, reorder: reordered.report, requests }, null, 2));
    return;
  }
  ...
  for (let n = 1; n <= DRAWS; n += 1) {
    for (const [arm, parts] of [["A", partsA], ["B", partsB]]) {
      const label = `${arm}${n}`;
      const file = `${label}-${arm === "A" ? "owner-proof" : "reordered-proof"}.png`;
      log("");
  ...
  const results = {
    contract: "designpro.call1-teaching-proof-order.v1",
  ...
  writeFileSync(join(OUT, "COMPARISON.md"), [
    "# Test 4 — teaching-proof centre order, raw pre-repair masters",
    "",
    "A = the owner teaching proof (HOOD > ROOF > REAR > FRONT). B = the same proof",
    "with only the HOOD and REAR blocks swapped (REAR > ROOF > HOOD > FRONT).",
    `Guide absent in both, no topology text. ${DRAWS} draws per arm, interleaved.`,
    "",
...
if (process.argv[1] && process.argv[1].endsWith("atlas-teaching-proof-order-ab.mjs")) {
  main().catch((error) => {
