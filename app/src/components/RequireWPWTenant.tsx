...
/**
 * RequireWPWTenant — gates access to the WPW-tenant Engine Room.
 *
 * Scoped to the WePrintWraps internal team + owner:
 *   lance@weprintwraps.com, brice@weprintwraps.com,
 *   jackson@weprintwraps.com, trish@weprintwraps.com,
 *   trish@restyleproai.com, tdill@restyleproai.com
 *
 * When we productize this as a paid tenant tier, replace the
 * email allowlist with an organization/subscription lookup.
 */
export const RequireWPWTenant = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const decide = (user: { email?: string } | undefined | null) => {
      ...
      setStatus(user && isWpwTenantMember(user.email) ? "authorized" : "denied");
    };
