...
function Step({ n, title, copy, children }: { n: number; title: string; copy: string; children: ReactNode }) {
  return (
    <article className="border wall-edge bg-[hsl(var(--wall-card))]">
      <div className="flex items-start gap-3 border-b wall-edge px-4 py-3">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center bg-blue-600 text-xs font-black text-white">{n}</span>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Step n={2} title="Touch 4 corners" copy="Four touches define the exact wall geometry.">
          <Frame label="Corner geometry">
            ...
            <span className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-blue-600 px-2.5 py-1 text-[10px] font-bold text-white shadow"><CornerDownRight className="mr-1 inline h-3 w-3" />4 corners mapped</span>
          </Frame>
        </Step>
        ...
        <Step n={3} title="1-touch masking" copy="Tap curtains, windows or objects once to protect them.">
          <Frame label="Protected">
            ...
            <div className="absolute left-[29%] top-[8%] h-[54%] w-[17%] border-2 border-cyan-300 bg-cyan-400/25 shadow-[0_0_0_1px_rgba(255,255,255,.7)]" />
            <div className="absolute right-[29%] top-[8%] h-[54%] w-[17%] border-2 border-cyan-300 bg-cyan-400/25 shadow-[0_0_0_1px_rgba(255,255,255,.7)]" />
            <div className="absolute left-[45%] top-[16%] h-[45%] w-[10%] border-2 border-cyan-300 bg-cyan-400/20" />
            <span className="absolute left-[28%] top-[30%] bg-cyan-500 px-2 py-1 text-[9px] font-bold text-white shadow">Protected curtain</span>
            <span className="absolute right-[27%] top-[46%] bg-cyan-500 px-2 py-1 text-[9px] font-bold text-white shadow">Protected curtain</span>
            <span className="absolute left-1/2 top-[12%] -translate-x-1/2 bg-white px-2 py-1 text-[9px] font-bold text-slate-900 shadow"><MousePointerClick className="mr-1 inline h-3 w-3 text-blue-600" />one touch</span>
          </Frame>
        </Step>
        ...
        <Step n={4} title="Preview on your wall" copy="Your design lands inside the geometry you marked.">
          <Frame label="On your wall">
            ...
            <span className="absolute bottom-2 right-2 bg-emerald-500 px-2 py-1 text-[9px] font-bold text-white shadow"><Eye className="mr-1 inline h-3 w-3" />exact placement</span>
          </Frame>
        </Step>
        ...
        <Step n={5} title="Print-ready panels" copy="Panelized at press width with ½″ perimeter bleed.">
          <Frame label="Production">
            ...
            <span className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-emerald-500 px-2 py-1 text-[9px] font-bold text-white shadow"><FileCheck2 className="mr-1 inline h-3 w-3" />3 panels · ½″ bleed</span>
          </Frame>
        </Step>
      </div>

      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-y wall-edge bg-[hsl(var(--wall-field))] px-4 py-3">
