  ...
  return (
    <section className="mt-8 space-y-6">
      ...
      <div className={`overflow-hidden ${WALL_CARD} p-0`}>
        <div className="flex flex-wrap items-end justify-between gap-4 border-b wall-edge bg-gradient-to-r from-sky-50 via-violet-50 to-fuchsia-50 p-5 md:p-6">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-blue-700">
              Printed by WePrintWraps
            </p>
            <h2 className="mt-1 text-xl font-bold wall-ink md:text-2xl">Add the printed wrap</h2>
            <p className="mt-1 max-w-md text-sm wall-muted">
              Your design on {WALL_FILM.name.replace('Avery Dennison® ', '')}, panelized and ready to hang.
            </p>
          </div>
          {quote && billing ? (
            <div className="text-right">
              <p className="text-3xl font-bold tabular-nums wall-ink md:text-4xl">
            ...
            <p className="text-sm wall-muted">Enter your wall size above.</p>
          )}
        </div>
        ...
        {billing && buy && (
          <div className="space-y-3 p-5 md:p-6">
            ...
            {buy.note && <p className="text-[11px] wall-muted">{buy.note}</p>}
      ...
      <div className={`${WALL_CARD} md:p-6`}>
        <h3 className="text-base font-bold wall-ink">What happens after you approve it</h3>
        <ol className="mt-4 grid gap-5 md:grid-cols-3">
          <li>
            <Layers className="h-5 w-5 text-blue-600" />
            <p className="mt-2 text-sm font-semibold wall-ink">Panelized to your wall</p>
            <p className="mt-1 text-xs leading-relaxed wall-muted">
          ...
          <li>
            ...
            <p className="mt-2 text-sm font-semibold wall-ink">A person checks it — within 24 hours</p>
            <p className="mt-1 text-xs leading-relaxed wall-muted">
          ...
          <li>
            ...
            <p className="mt-2 text-sm font-semibold wall-ink">You get the actual files</p>
            <p className="mt-1 text-xs leading-relaxed wall-muted">
        ...
        <p className="mt-5 flex items-start gap-2 rounded-xl bg-[hsl(var(--wall-field))] p-4 text-xs leading-relaxed wall-muted">
          <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />
          <span>
            <strong className="wall-ink">That 24-hour window is the point, not a delay.</strong> A design tool
      ...
      <details open className={`${WALL_CARD} md:p-6`}>
        <summary className="cursor-pointer text-base font-bold wall-ink">The material</summary>
        <dl className="mt-4 grid gap-x-8 gap-y-2 text-sm sm:grid-cols-2">
          {SPECS.map(([label, value]) => (
            <div key={label} className="flex flex-wrap justify-between gap-x-4 border-b border-slate-100 pb-2">
              <dt className="wall-muted">{label}</dt>
              <dd className="font-medium wall-ink">{value}</dd>
            </div>
          ))}
        </dl>
      </details>
