...
// Header brand mark — the official two-tone RestyleProAI logo (sprocket
...
const PLATFORM_LOGO_URL =
  'https://restyleproai.com/sprocket/restyleproai-logo.png';

// Footer credit reuses the same brand mark, sized smaller.
const PLATFORM_FOOTER_LOGO_URL = PLATFORM_LOGO_URL;

const PLATFORM_GALLERY_URL = 'https://restyleproai.com/gallery';
const PLATFORM_PRICING_URL = 'https://restyleproai.com/pricing';
...
export interface QuotePDFData {
   ...
   * top-corner co-brand strip as "RestyleProAI × weprintwraps.com" with
...
export async function generateQuotePDF(data: QuotePDFData): Promise<void> {
  ...
  // ─── PLATFORM CO-BRAND ─────────────────────────────────────────────
  // Two-tone RestyleProAI logo (sprocket + wordmark) sits at the top
  ...
  if (rocketData) {
    ...
    pdf.text('RestyleProAI', coBrandX, y);
    coBrandX += pdf.getTextWidth('RestyleProAI') + 3;
  }
  ...
  if (tierCards.length > 0) {
    ...
    if (y + panelH < pageH - 16) {
      ...
      pdf.setTextColor(DARK.r, DARK.g, DARK.b);
      pdf.text(
        'RestyleProAI — like having a graphic designer at your fingertips',
        mL,
        y + 4,
      );
      pdf.setFont('helvetica', 'normal');
  ...
  // Small RP sprocket + "Generated by RestylePro™" — bottom center.
