...
/**
 * The AI's QuickQuotes live in THIS app's quotes table, and every quote row
 * carries a share_token — /q/<token> is the exact-quote page. This hook maps
 * quote numbers -> tokens so the dashboard links open THE quote, not the
 * /quotes hub.
 */
function useQuoteShareLinks(quoteNumbers: string[]) {
  const key = [...quoteNumbers].sort().join(",");
  const { data } = useQuery({
    queryKey: ["quote-share-links", key],
    enabled: quoteNumbers.length > 0,
    queryFn: async () => {
      const { data: rows } = await supabase
        .from("quotes")
        .select("quote_number, share_token")
        .in("quote_number", quoteNumbers);
      const map: Record<string, string> = {};
      for (const r of rows || []) if (r.quote_number && r.share_token) map[r.quote_number] = r.share_token;
      return map;
    },
  });
  return data || {};
...
function quoteUrlFor(qn: string | null | undefined, links: Record<string, string>) {
  return qn && links[qn] ? `/q/${links[qn]}` : "/quotes";
}
...
interface LeadQuote {
  ...
  /** Real QuickQuote number (quotes table, tool_source 'AI Assistant') — v25 bridge. */
  quote_number?: string | null;
  rp_quote_id?: string | null;
}

/**
 * Draft editing is PLAIN TEXT, not HTML. The editable part is everything
 * before the quote button / signature; those are locked and re-attached
 * verbatim so an edit can never lose the cart link or the branded signature.
 */
const EDIT_SPLIT_RE = /<!--WPW_QUOTE_START-->|<!--WPW_SIG-->/;
function splitDraft(html: string) {
  const m = EDIT_SPLIT_RE.exec(html || "");
  return m ? { body: html.slice(0, m.index), rest: html.slice(m.index) } : { body: html || "", rest: "" };
}
function draftHtmlToText(html: string) {
  return (html || "")
    .replace(/<\s*br\s*\/?\s*>/gi, "\n")
    .replace(/<\/p>/gi, "\n\n")
    .replace(/<[^>]+>/g, "")
...
function draftTextToHtml(text: string) {
...
interface QuoteStatRow {
  ...
  metadata?: { cart_url?: string | null; quote_number?: string | null } | null;
  /** Joined by the agent (v23+): the email this quote was drafted for. */
  email?: { lead_reply_id: string; subject: string | null; status: string; mailbox: string; received_at: string | null } | null;
  created_at: string;
}
...
function QuoteAnalyticsCard() {
  ...
  const shareLinks = useQuoteShareLinks(
    (data?.recent || []).map((q) => q.metadata?.quote_number).filter(Boolean) as string[]
  );
  return (
    <Card className="rounded-2xl bg-white border border-slate-300 border-l-4 border-l-emerald-500 shadow-[0_1px_2px_rgba(15,23,42,0.05),0_12px_32px_-16px_rgba(15,23,42,0.22)]">
      <CardContent className="py-4 space-y-3">
        ...
        {/* EVERY quote the AI made, one row per email — this table is the answer
            to "I need to see the quotes AI is making per email". */}
        <div className="rounded-xl bg-white ring-1 ring-slate-300 overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-100 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                <th className="px-3 py-2">Date</th>
                <th className="px-3 py-2">Customer</th>
                <th className="px-3 py-2">Email it's attached to</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {(data?.recent || []).slice(0, open ? 200 : 25).map((q) => (
                <tr key={q.id} className="hover:bg-slate-50">
                  <td className="px-3 py-2 whitespace-nowrap text-slate-500">
                    {new Date(q.created_at).toLocaleDateString()}
                  </td>
                  <td className="px-3 py-2">
                    <div className="font-medium text-slate-900">{q.customer_name || q.customer_email}</div>
                    <div className="text-xs text-slate-500">{q.customer_email}</div>
                  </td>
                  <td className="px-3 py-2 max-w-[220px]">
                    <div className="truncate text-slate-700" title={q.email?.subject || ""}>
                      {q.email?.subject || <span className="text-slate-400">—</span>}
                    </div>
                    {q.email && (
                      <div className="text-xs text-slate-400">
                        {q.email.mailbox.replace("@weprintwraps.com", "@")} · {q.email.status.replace("_", " ")}
                      </div>
                    )}
                  </td>
                  <td className="px-3 py-2 whitespace-nowrap">
                    {q.metadata?.quote_number ? (
                      <a
                        href={quoteUrlFor(q.metadata.quote_number, shareLinks)}
                        target="_blank"
                        rel="noreferrer"
                        className="font-semibold text-emerald-700 hover:underline"
                        title="Open this exact quote"
                      >
                        {q.metadata.quote_number} ↗
                      </a>
                    ) : (
                  ...
                  <td className="px-3 py-2 text-right text-slate-700">{q.sqft.toLocaleString()}</td>
                  <td className="px-3 py-2 text-right text-slate-700">
                    {q.price_per_sqft ? `$${Number(q.price_per_sqft).toFixed(2)}` : "—"}
                  </td>
                  ...
                  <td className="px-3 py-2">
                    {q.converted_to_order ? (
                      <Badge className="bg-purple-100 text-purple-700 border-purple-200">Won</Badge>
              ...
              {!data?.recent?.length && (
                <tr>
                  <td colSpan={9} className="px-3 py-4 text-center text-slate-400">No quotes yet.</td>
                </tr>
              )}
            </tbody>
          </table>
...
function DraftCard({ row, onDone }: { row: LeadReply; onDone: () => void }) {
  ...
  const shareLinks = useQuoteShareLinks(quote?.quote_number ? [quote.quote_number] : []);
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            {quote && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 ring-1 ring-emerald-200 px-3 py-1 text-sm font-semibold text-emerald-700">
                {quote.quote_number ? `${quote.quote_number} · ` : "Quote: "}
                {quote.sqft.toLocaleString()} sq ft · ${quote.total.toLocaleString()}{" "}
            ...
            {quote?.quote_number && (
              <a
                href={quoteUrlFor(quote.quote_number, shareLinks)}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center rounded-full bg-white ring-1 ring-emerald-300 px-3 py-1 text-sm font-semibold text-emerald-700 shadow-sm hover:bg-emerald-50"
                title="Open this exact quote"
              >
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {pending && (
              <Button
                ...
                onClick={() => {
                  if (preview) {
                    const { body, rest } = splitDraft(html);
                    setEditText(draftHtmlToText(body));
                    setEditRest(rest);
                  }
                  setPreview(!preview);
                }}
            ...
            <>
              <Textarea
                ...
                onChange={(e) => {
                  ...
                  setEditText(v);
                  setHtml(draftTextToHtml(v) + (editRest ? `\n${editRest}` : ""));
                }}
                rows={12}
                className="bg-white text-slate-900 border-slate-200 text-sm leading-relaxed"
                placeholder="Write the reply like a normal email — blank line = new paragraph"
              />
              <div className="mt-1.5 text-[11px] text-slate-500">
                Plain text — blank line starts a new paragraph. The order button and branded
                signature are locked and re-attached automatically, so edits can't break them.
              </div>
            </>
          )}
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-200 text-slate-900 px-4 py-8 pb-32 sm:px-8">
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <SectionTitle>Quotes the AI made — per email</SectionTitle>
        <QuoteAnalyticsCard />
