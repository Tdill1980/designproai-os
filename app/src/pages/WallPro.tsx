...
export default function WallPro({ brand = 'designpro' }: { brand?: WallBrandKey } = {}) {
  ...
  async function openDesignProof() {
    if (!photo || !preview || !canvas.current || rendering || !artwork || !wallLocated || !billing || scaleSettling || preparingProof) return;
    const version = previewVersion.current;
    const snapshot = { name: zoneLabel ? `${name} — ${zoneLabel}` : name, sourceId: currentVersionId || projectId,
