...
test("ordered migration chain retains existing production boundaries and appends output graphs and parent-bound revisions", () => {
  ...
  assert.deepEqual(names.slice(-124), [
    ...
    // (owner, 2026-09-12: "we can't risk going 100% ai").
    "20260912240000_wallpro_human_release_gate.sql",
    // Multi-tenant phase 1, ported from restylepro-os: the franchise/shop spine
    // the white-label WallPro link and the vehicle suite both sit on.
    "20260913000000_designpro_multi_tenant_phase1.sql",
    "20260914160000_wallpro_catalog_scenes.sql",
    "20260914200000_designpro_atlas_refusals.sql",
    "20260914230000_designpro_call11_receipt_branch.sql",
    "20260915010000_designpro_production_run_generation_identity.sql",
    // Try free, and CommercialPro members design free. Stamped 20260915020000
    // rather than 20260914230000, which is where it was written: the Call 11
    // receipt branch landed on main with that exact stamp while this branch was
    // open, and supabase_migrations.schema_migrations keys on the version, so
    ...
    // The proof band becomes curator-managed: a before/after stops
    // needing a script, a hand-edited constant and a release.
    "20260915030000_wallpro_proof_band.sql",
    ...
    // My ShopFlow, the WePrintWraps account page (#416): CommercialPro
    ...
    // A paid WallPro design gets a real, sequential order number -- the
    ...
    "20260917040522_wallpro_landing_media.sql",
    "20260917230000_designpro_server_owned_handoff.sql",
    ...
    // `proof.assemble` may name its own run's master. Without it the panel-proof
    // pair does all of its work -- sheet cut, eleven sibling panels stored,
    // master assembled -- and then cannot be FINISHED, because the finish RPC
    // let only master.assemble and master.composite write the run's master
    // columns and the run's CHECK refuses to complete without them. Measured
    // that way on the real migration: node stuck running, lease expired, work
    // repeated, caller timed out. Same NULL guard as master.composite carries.
    "20260919190000_designpro_atlas_call1_proof_assemble_master.sql",
    "20260920011000_designpro_early_panel_proof.sql",
    "20260920022906_designpro_panel_proof_logo_handoff.sql",
    "20260920025200_designpro_panel_proof_inventory_shape.sql",
    "20260920042000_designpro_revision_pinned_purchase.sql",
    "20260920043000_designpro_enhance_terminal_lease_release.sql",
    "20260920080000_designpro_call1_sheet_immediate.sql",
    "20260920093000_designpro_composed_call1_only.sql",
    "20260920113000_designpro_paid_pdf_outputs.sql",
    "20260920154447_designpro_prompt_record.sql",
    // #583: the asset-path validator service_role could not execute, and the
