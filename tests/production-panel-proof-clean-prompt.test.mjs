...
const PERSONA='ROLE: Senior commercial wrap-artwork designer. Create the clean printed BACKGROUND ARTWORK layer for six vehicle panel cells.';
...
test('active separated Call 1 injects exact persona and omits contradictory branded generation directions',async()=>{
  ...
  assert.match(prompt,/OUTPUT TARGET:/);
  assert.match(prompt,/deterministic compositor owns all Production Panel Proof document chrome/);
  ...
  assert.match(prompt,/Your deliverable is the six clean background artworks/);
  const systemAuthored = prompt.replace(BODY.creativeDirection,'');
  assert.doesNotMatch(systemAuthored,/\b(?:no|not|never|don't|do not|without|exclude)\b/i,
    'system-authored separated-artwork prompt uses positive target language');
