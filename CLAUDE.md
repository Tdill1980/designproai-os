...
## 🚗 CALL 1 IS THE HERO ON THE VEHICLE; EVERY FLAT IS DERIVED FROM A VIEW OF IT (owner, Trish 2026-09-22 — "This is my quality")

**This supersedes the same-day ruling below that the panel production proof is
the only Call 1.** The first real generation on that route (request
`0f53d4e7`, 2021 F150, Botanical Gardens brief with a home photo and a custom
logo) came back as gradient panels, generic typeset lettering, a doubled logo
and a 115 s Call 1. Owner: *"these are not my design edge functions my system
...
Fitness van HERO and its flat driver panel, the McLaren seven-view Design
Approval Proof, two three-zone sheets. Diagnosed from the row and the code, not
from impressions: a document that asks the model for six ~1200×300 px panels
inside a 3:2 sheet cannot carry a design, and the Zone 1 it showed was
code-composited, not drawn.

**The live Call 1 is now the hero-driver cascade with hero-first on EVERY AI
surface** (`runtime/atlas-hero-driver.cjs`, `HERO_VIEW_SURFACES` =
driver/hood/front/rear/roof; `runtime/atlas-call1-graph.cjs`
`compileHeroDriverGraph({heroFirst:true})`), routed explicitly by
`generateOrReuseFlatAtlas` for every unnamed request — `heroFirst: true` rides
the routing and the run DEFINITION, so no deploy flag selects or deselects it.
The RestylePro order, as three waves:

| wave | requests | what |
|---|---|---|
| 1 | 1 | the HERO: `design-panel-ai-generate` `atlas-author`, persona brain, the customer's RAW brief, `viewType: side`, `atlasFlatMaster: false` (camera + scene + studio), **4K**, no reference, no history |
| 2 | 5, parallel | the driver FLATTEN of the hero (ported `renderFlatTile`, replays the hero's exchange + thought signature) beside hood / front / rear / roof VIEWS — each shown the staged hero by content-addressed identity and told: same wrap, its own locked camera (`VIEW_TYPES`: hood_detail / front / rear / roof), 2K |
| 3 | 4, parallel | the four flattens (each shown only its own view) beside the passenger flop (code) |

Then the SAME gates, the same assembly, and the **three-zone Production Panel
Proof DERIVED from the accepted master** (`assemblePanelProofMaster
({documentOnly})`, `metadata.panelProofAuthoring.topology =
"derived-from-accepted-master"`). Critical path: three sequential model calls.

**Three things found and fixed on the way, all of them the "lock encoded the
bug" shape this file keeps recording:**
1. **The "hero view" was never a photograph of the vehicle.** The edge read
   `atlasFlatMaster: !heroFlatten` — TRUE on the view leg — which with
   `atlasHeroSurface` set selected `atlasHeroScene` ("ONE FLAT PRINTED SHEET")
   at 16:9, while the comment beside it and the lock pinning it both said
   "asks for the vehicle". Hero-first's 0/3 was measured on a flat sheet
   flattened again. It is FALSE on both legs now, with the camera per surface.
2. **A non-driver view without the hero would be its own design** (RULE 0.0),
   so the edge refuses it before a request is spent
   (`atlas_author_view_requires_hero_reference`); a hero reference anywhere
   else is refused too; the driver hero takes none. Both the graph fixture and
   the in-process fixture enforce those doors.
3. **The hero authors its own lettering.** `heroRequestBody` sends no
   `cleanBase` on hero-first, `compileHeroDriverGraph` compiles NO element
   node for it, and the post-acceptance compositor records
   `elementGraph.skipped = "hero_first_authors_its_own_lettering"` — a typeset
   lockup over brain-lettered flanks is the "generic text" the owner rejected,
   and would print the name twice. The element graph is intact on the
   retained single-call shape.

**Revisions run the same cascade**: the parent's driver hero
(`metadata.heroDriverAuthoring.views.driver`, the accepted master as the
stand-in for a parent authored before this) is staged into the edge's input
prefix and attached to the HERO request only, with the customer's instruction
(`atlasHeroRevisionPrompt`); the views and flattens then follow from the edited
hero. `flat_atlas_hero_edit_unsupported` remains only on the single-call shape.

**Dead but retained, by name:** the panel-proof route (a caller that names
`authoringTopology: "panel-proof"` still gets it; stored runs stay readable and
resumable), hero-driver's single-call shape, field-first, six-surface and its
one-field fail-over. A refused HERO still fails over to six-surface, recorded
as `authoringFailover` — the customer never ends with nothing.

**Deploy facts:** runtime + edge (`design-panel-ai-generate`, prompt version
`atlas-author-hero-first.20260922.v6-every-surface-hero-view`, refused on
mismatch) must ship together; no migration (`surface.<key>.view` keys are
admitted by the existing `node_key` CHECK). The flags are receipts only:
`DESIGNPRO_ATLAS_HERO_FIRST`, `_PANEL_PROOF` on or off do not change routing.
The canary (`scripts/production-canary.mjs`) reads the hero-first shape: ≤15
requests on the contract (+2 across a fail-over), a latency budget of three
sequential calls, the derived three-zone document, no element composite.

**NOT PROVEN, and must not be claimed:** no real generation has run on this
path. Locked by `tests/hero-first-is-the-only-call1.test.mjs` (routing by
execution), `tests/atlas-hero-driver-topology.test.mjs`,
`tests/atlas-call1-graph.test.mjs` (the twelve-node shape on the real
migration), `tests/atlas-element-graph.test.mjs`,
`tests/production-canary-contract.test.mjs`. Acceptance is the owner's eye on
the exported hero, the views and the six panels of a fresh generation.
