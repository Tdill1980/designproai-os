...
test("one canonical policy includes all nineteen runtime files and five deploy controls", () => {
  assert.equal(fixed.filter((name) => name.startsWith("runtime/")).length, 19);
  for (const name of [
    "runtime/resend-transport.cjs", "runtime/wrapbox-delivery.cjs", "runtime/zip-spool.cjs",
    "runtime/gemini-flat-wrap.cjs", "runtime/flat-wrap-layout.cjs", "runtime/proof-sheet.cjs", "runtime/topaz-upscale.cjs", "runtime/view-angles.cjs", "runtime/generation-provider.cjs", "runtime/generation-engine.cjs",
