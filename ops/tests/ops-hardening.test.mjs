...
test("one canonical policy includes every required runtime file and five deploy controls", () => {
  ...
  for (const name of [
    ...
    // The design contract around the frozen camera contract. Without it the
    // worker cannot assemble a prompt, so a release missing it has a generation
    // queue whose executor dies on its first claim.
    "runtime/design-prompt-os.cjs",
