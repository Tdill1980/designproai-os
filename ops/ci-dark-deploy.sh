...
if [[ -L /opt/designproai-os/current ]] && \
  ...
  # (ATLAS_PANEL_FINISH, ATLAS_PANEL_REFINE, ATLAS_TOPOLOGY, ATLAS_CALL1_GRAPH, ATLAS_FIELD_FIRST,
  # ATLAS_HERO_FIRST, ATLAS_ELEMENT_GRAPH, ATLAS_PANEL_PROOF)
  # only ever reach the droplet through the env writer below, which this
  # branch used to skip -- so
  ...
  if [[ -n ${ATLAS_PANEL_FINISH:-}${ATLAS_TOPOLOGY:-}${ATLAS_CALL1_GRAPH:-}${ATLAS_FIELD_FIRST:-}${ATLAS_HERO_FIRST:-}${ATLAS_ELEMENT_GRAPH:-}${ATLAS_PANEL_PROOF:-}${ATLAS_PANEL_REFINE:-} ]]; then
