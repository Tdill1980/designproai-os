...
def validate(runtime_path: Path, gateway_path: Path) -> None:
    ...
    for flag, allowed in (("DESIGNPRO_ATLAS_TOPOLOGY", {"six-surface", "hero-driver"}), ("DESIGNPRO_ATLAS_CALL1_GRAPH", {"on", "off"}), ("DESIGNPRO_ATLAS_FIELD_FIRST", {"on", "off"}), ("DESIGNPRO_ATLAS_HERO_FIRST", {"on", "off"}), ("DESIGNPRO_ATLAS_ELEMENT_GRAPH", {"on", "off"})):
        if flag in runtime:
